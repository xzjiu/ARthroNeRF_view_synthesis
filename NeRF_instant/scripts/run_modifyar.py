...
def parse_args():
	...
	parser.add_argument("--row", default="c:/Users/camp/Documents/Xinrui Zou/Tool-ROM/tool_ar1.rom", help="used only when there is a ndi")
	parser.add_argument("--row2", default="c:/Users/camp/Documents/Xinrui Zou/Tool-ROM/tool_new_offset.rom", help="Path to the second ROM file. Used only when NDI tracking is enabled.")
			...
			while testbed.frame():
				...
				if args.gui and args.ndi:
					...
					if not np.isnan(tracking[0][0][-1]):
						...
						if transform_matrix is not None and frame2 is not None:
							# Calculate the inverse of Frame 1's matrix
							frame1_inv = np.linalg.inv(transform_matrix)

							# Calculate the transformation of Frame 2 relative to Frame 1
							frame2_relative_to_frame1 = np.dot(frame1_inv, frame2)
						tool_dis = frame2_relative_to_frame1[:3, 3]
						tool_dis[2] = tool_dis[2] + 100
						testbed.frame2 = tool_dis
						...
						vector_up = [rotation_matrix[1, 1], rotation_matrix[2, 1],rotation_matrix[0, 1]]
						testbed.up_dir = vector_up
						testbed.scale = tran_vec[2]*0.0036754*0.05
