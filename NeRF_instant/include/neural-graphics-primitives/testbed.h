...
class Testbed {
private:
    bool showCircleOverlay;
	glm::vec3 g_lastViewDir;
...
public:
	...
	vec3 ndi_rotation_matrix() const { return ndi_rotation; }
	void set_ndi_rotation_matrix(const vec3& m_ndi_rotation);
	...
	vec3 ndi_rotation = vec3(1.0f);
	vec3 ndi_camera_1 = vec3(1.0f);
	vec3 ndi_camera_2 = vec3(1.0f);
	vec3 ndi_camera_3 = vec3(1.0f);
	
	vec3 pressnum = vec3(1.0f);
	vec3 frame2 = vec3(1.0f);
	float usernum = 0.f;
	float timer = 0.f;
	float tasktime = 0.f;
	...
	void blit_texture_for_tool(const Foveation& foveation, GLint rgba_texture, GLint rgba_filter_mode,  GLint framebuffer, const ivec2& offset, const ivec2& resolution);
	...
	std::vector<std::shared_ptr<GLTexture>> m_tool_pip_render_textures;
	...
	// std::vector<shared_ptr<CudaRenderBuffer>> m_tool_pip_render_buffer;
